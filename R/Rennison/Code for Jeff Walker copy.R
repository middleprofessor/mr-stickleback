...
# as follows:
# ----------
x <- read.csv("data/Rennison et al archive.csv", stringsAsFactors=FALSE) #jaw april 08, 2017
...
genotype_frequency <- function(x){
  # returns the frequency of the different levels of genotype
  table(x)/length(x)
}

resample_rennison <- function(x, which_month){
  # returns a resampled matrix of adjusted plates and genotype for a month
  # month is the level to compute the summary
  freqs <- genotype_frequency(x[month==which_month & genotype!="LL", genotype])
  month_freq <- c(freqs['CC'], freqs['CL']) # ordered by CC=0, CL=1
  n <- nrow(x[month==which_month & genotype!="LL", ])
  CC_adj_plates <- x[genotype=='CC' & month==which_month, adj.plates]
  CL_adj_plates <- x[genotype=='CL' & month==which_month, adj.plates]
  
  geno <- sample (c(0,1), size=200, prob=rep(month_freq), replace=TRUE)
  geno2 <- sample(geno, size=n, prob=NULL, replace=FALSE ) # note this is redundant
  # if i==1 then use actual assignment
  CCgeno1 <- subset(geno2, geno2 ==0)
  sum_CCgeno1 <- length(CCgeno1)
  adjplates <- vector(length=n) 
  adjplates[geno2==0] <- sample(x = CC_adj_plates, size = sum_CCgeno1, replace=TRUE, prob = NULL)
  
  CLgeno1 <- subset(geno2, geno2 ==1)
  sum_CLgeno1 <- length(CLgeno1)
  adjplates[geno2==1] <- sample(x = CL_adj_plates, size = sum_CLgeno1, replace=TRUE, prob = NULL)

  x1 <- cbind(adjplates, geno2)
  return(x1)
}

rennison_mv_selection_differential_rep <- function(x){
  # rep of rennison's but turning it's repetition into functions
  # x is a data.table
  
  x <- data.table(read.csv("data/Rennison et al archive.csv", stringsAsFactors=FALSE))
  x[, month:=factor(month, c("06-Sep", "06-Oct", "06-Nov" ))]
  ...
  oct_nov_rep <- sept_oct_rep <- matrix(NA, nrow=n_reps,ncol=6)
  colnames(oct_nov_rep) <- colnames(sept_oct_rep) <-c("s1","s2","beta1","beta2","beta.std1","beta.std2")
  
  for(i in 1:n_reps){
    x1.sept <- resample_rennison(x, "06-Sep")
    means.sept <- apply(x1.sept, 2, FUN=mean, na.rm=TRUE)
    P.sept <- var(x1.sept)
    x1.oct <- resample_rennison(x, "06-Oct")
    means.oct <- apply(x1.oct, 2, FUN=mean, na.rm=TRUE)
    P.oct <- var(x1.oct)
    x1.nov <- resample_rennison(x, "06-Nov")
    means.nov <- apply(x1.nov, 2, FUN=mean, na.rm=TRUE)
    P.nov <- var(x1.nov)

    sept_oct_rep[i,] <- unlist(beta.before.after(means.sept, means.oct, P.sept))
    oct_nov_rep[i,] <- unlist(beta.before.after(means.oct, means.nov, P.oct))
  }

  quantile(sept_oct_rep[,5], probs=c(0.025, 0.5, 0.975))
  quantile(sept_oct_rep[,6], probs=c(0.025, 0.5, 0.975))
  quantile(oct_nov_rep[,5], probs=c(0.025, 0.5, 0.975))
  quantile(oct_nov_rep[,6], probs=c(0.025, 0.5, 0.975))
  
}
  
rennison_mv_selection_differential_orig <- function(x){
  ...
  # *********JAW editied because months in data are in format "DD-MM"*****************
  
  CCs <- subset(CC,CC$month=="06-Sep")
  #mean(CCs$adj.plates)
  #22.1
  
  CLs <- subset(CL,CL$month=="06-Sep")
  #mean(CLs$adj.plates)
  #19.74286
  
  CCo <- subset(CC,CC$month=="06-Oct")
  #mean(CCo$adj.plates) 
  #25.82353
  
  CLo <- subset(CL,CL$month=="06-Oct")
  #mean(CLo$adj.plates)
  #19.14286
  
  CCn <- subset(CC,CC$month=="06-Nov")
  #mean(CCn$adj.plates)
  #24.31429 
  
  CLn <- subset(CL,CL$month=="06-Nov")
  #mean(CLn$adj.plates)
  #18.72414
  ...
  septfreq <- c(0.47, 0.53) # for available data: 46.2, 53.8 JAW April 09, 2018
  octfreq <- c(0.7, 0.3) # for available data: .62, .38 JAW April 09, 2018
  novfreq <- c(0.54, 0.46) # for available data: .55, .45 JAW April 09, 2018
  
  # redo with frequencies computed from data here JAW April 09, 2018
  # septfreq <- c(nrow(CCs), nrow(CLs))/(nrow(CCs) + nrow(CLs))
  # octfreq <- c(nrow(CCo), nrow(CLo))/(nrow(CCo) + nrow(CLo))
  # novfreq <- c(nrow(CCn), nrow(CLn))/(nrow(CCn) + nrow(CLn))
  ...
  n_reps <- 20000
  sept <- matrix(NA, nrow=n_reps,ncol=2)
  colnames(sept) <- c('sadjplates','septgeno2')
  oct <- matrix(NA, nrow=n_reps,ncol=2)
  colnames(oct) <- c('oadjplates','octgeno2')
  nov <- matrix(NA, nrow=n_reps,ncol=2)
  colnames(nov) <- c('nadjplates','novgeno2')
  ...
  # *** JAW 4/0/18 modified code to analyze actual assignment in iteration (i=) 1. Also, there are many lines to "sample" a new frequency of CC and CL genotypes each iteration, as a sample of the original frequency. And then sample these many CC and these many CL. But this code isn't necessary, Simply re-sampling rows but ignoring genotype assigment does this automatically.
  ...
  for(i in 1:n_reps) {
    ...
    if(i==1){ # JAW 04/09/18 - to get estimates of actual data
      septgeno2 <- c(rep(0, length(CCs$adj.plates)), rep(1, length(CLs$adj.plates))) # cc cl
      sadjplates[septgeno2==0] <- CCs$adj.plates
      sadjplates[septgeno2==1] <- CLs$adj.plates
    }
    
    x1.sept<-cbind(sadjplates, septgeno2) 
    means.sept <- apply(x1.sept, 2, FUN=mean, na.rm=TRUE)
    P.sept <- var(x1.sept)
    sept[i,] <- means.sept #JAW 04/09/18
    ...
    if(i==1){# JAW 04/09/18 - to get estimates of actual data
      octgeno2 <- c(rep(0, length(CCo$adj.plates)), rep(1, length(CLo$adj.plates))) # cc, cl
      oadjplates[octgeno2==0] <- CCo$adj.plates
      oadjplates[octgeno2==1] <- CLo$adj.plates
    }
    ...
    novgeno2 <- sample(novgeno, size=64, prob=NULL, replace=FALSE ) # BUG changed 74 to 64
    ...
    if(i==1){# JAW 04/09/18 - to get estimates of actual data
      novgeno2 <- c(rep(0, length(CCn$adj.plates)), rep(1, length(CLn$adj.plates)))
      nadjplates[novgeno2==0] <- CCn$adj.plates
      nadjplates[novgeno2==1] <- CLn$adj.plates
    }
    
    x1.nov<-cbind(nadjplates, novgeno2) 
    means.nov <- apply(x1.nov, 2, FUN=mean, na.rm=TRUE)
    P.nov <- var(x1.nov)
    nov[i,] <- means.nov #JAW 04/09/18
  ...
  round(apply(sept_oct, 2, mean), 3)
  round(apply(oct_nov, 2, mean), 3)
 
  #SeptOct
  #Phenotype
  mean(sept_oct[,5])
  mean(sept_oct[, "beta.std1"])
  #0.3435019
  #Command to retrive the 95% Confidence Interval 
  quantile(sept_oct[,5], probs=c(0.025, 0.5, 0.975)) # JAW April 09, 2018
  
  #Genotype
  mean(sept_oct[,6])
  #-0.4164707
  quantile(sept_oct[,6], probs=c(0.025, 0.5, 0.975))# JAW April 09, 2018
  
  #OctNov
  #Phenotype
  mean(oct_nov[,5])
  #-0.2104259
  quantile(oct_nov[,5], probs=c(0.025, 0.5, 0.975))# JAW April 09, 2018
  
  #Genotype
  mean(oct_nov[,6])
  #0.2565881
  quantile(oct_nov[,6], probs=c(0.025, 0.5, 0.975))# JAW April 09, 2018
...
# JAW April 09, 2018 test

niter <- 10000
n_0a <- numeric(niter)
n_0b <- numeric(niter)
n_0c <- numeric(niter)
raw <- c(rep(0,30), rep(1,35))
for(iter in 1:niter){
  septgeno <- sample (c(0,1), size=200, prob=rep(septfreq), replace=TRUE)
  septgeno2a <- sample(septgeno, size=65, prob=NULL, replace=FALSE )
  septgeno2b <- sample(c(0,1), size=65, prob=septfreq, replace=TRUE )
  septgeno2c <- sample(raw, replace=TRUE)
  n_0a[iter] <- length(septgeno2a[septgeno2a==0])
  n_0b[iter] <- length(septgeno2b[septgeno2b==0])
  n_0c[iter] <- length(septgeno2c[septgeno2c==0])
}
septfreq
mean(n_0a/65)
mean(n_0b/65)
...
n_0b <- numeric(niter)
n_0c <- numeric(niter)
raw <- c(rep(0,34), rep(1,21))
for(iter in 1:niter){
  octgeno <- sample (c(0,1), size=200, prob=rep(octfreq), replace=TRUE)
  octgeno2a <- sample(octgeno, size=55, prob=NULL, replace=FALSE )
  octgeno2b <- sample(c(0,1), size=55, prob=octfreq, replace=TRUE )
  octgeno2c <- sample(raw, replace=TRUE)
  n_0a[iter] <- length(octgeno2a[octgeno2a==0])
  n_0b[iter] <- length(octgeno2b[octgeno2b==0])
  n_0c[iter] <- length(octgeno2c[octgeno2c==0])
}
octfreq
mean(n_0a/55)
mean(n_0b/55)
